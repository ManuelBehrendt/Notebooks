#!/usr/bin/env python3
"""Build 03_gravity_Get_Subregions.ipynb (and a pre-flight .jl of its code cells).

Usage:
    python3 build_03_gravity.py            # write notebook + preflight
    python3 build_03_gravity.py --preflight-only
"""
import json, sys, os

OUT_NB = "/Volumes/FASTStorage/Simulations/Mera-Docs/version_1.1/03_gravity_Get_Subregions.ipynb"
OUT_JL = os.path.join(os.path.dirname(os.path.abspath(__file__)), "preflight_gravity.jl")
...
BANNER = "# ── figure code from here: panels, overlays, colorbars — no new Mera concepts ──"
...
def build():
    ...
    return {
        "cells": cells,
        "metadata": {
            "kernelspec": {"display_name": "Mera-Docs 1.12 (4t) 1.12",
...
def preflight():
    parts = ["# AUTO-GENERATED pre-flight: every code cell of 03_gravity, in order.\n",
             "# Run as a plain script to catch API errors before touching a kernel.\n\n"]
    parts.append("__t0 = time()\n")
    for i, (kind, text) in enumerate(CELLS):
        if kind == "code":
            parts.append("\n# ===================== cell %d =====================\n" % i)
            parts.append(text)
            parts.append('\nprintln(">>> cell %d ok  (", round(time()-__t0, digits=1), " s)")\n'
                         '__t0 = time()\n' % i)
    return "".join(parts)
...
if __name__ == "__main__":
    ...
    if "--preflight-only" not in sys.argv:
        ...
        print("wrote %s: %d cells (%d code, %d md)" % (OUT_NB, len(CELLS), n_code,
                                                       len(CELLS) - n_code))
